 ...
 * @return string
 */
function onestore_get_search_form_add_icon( $form ) {
	$form = preg_replace( '/placeholder="(.*?)"/', 'placeholder="' . esc_attr__( 'Search&hellip;', 'onestore' ) . '"', $form );
	$form = preg_replace( '/<\/form>/', onestore_icon( 'search', array( 'class' => 'onestore-search-icon' ), false ) . '</form>', $form );

	return $form;
}
add_filter( 'get_search_form', 'onestore_get_search_form_add_icon' );
